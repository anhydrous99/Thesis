...
PPO-Clip replaces the Policy Gradient Objective function, equation~\ref{eq:REINFORCE_objective}, with the Clipped Surrogate Objective, \begin{equation*}
	\nabla J(\theta)=\mathbb{E}[\min (r(\theta)A_t, \text{clip}(r(\theta), 1-\epsilon, 1+\epsilon) A_t)]
...
PPO-Penalty, on the other hand, uses a penalty to the KL-Divergence called the Adaptive KL Penalty Coefficient, $$\nabla J(\theta)=\mathbb{E}\big[r(\theta)A_t - \beta KL[\pi_{\theta_\text{old}}(\cdot | s_t), \pi_\theta (\cdot | s_t)]\big].$$ Here, $KL$ is the KL-Divergence and $\beta$ is a hyper-parameter that is updated via 
\begin{equation*}
